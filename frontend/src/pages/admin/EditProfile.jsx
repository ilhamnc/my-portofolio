...
export default function EditProfile() {
  ...
  useEffect(() => {
    fetch('https://my-portofolio-api.vercel.app/api/profile').then(res => res.json()).then(data => { if(data.id) setFormData(data); });
  }, []);

  const handleSubmit = async (e) => {
    e.preventDefault();
    await fetch('https://my-portofolio-api.vercel.app/api/profile', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(formData) });
    navigate('/admin');
  };
